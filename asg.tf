...
locals {
  template_ami_id = coalesce(lookup(var.cluster_instance_amis, var.region), data.aws_ami.amazon_linux_2.image_id)
  template_cluster_user_data = coalesce(var.cluster_instance_user_data_template, templatefile("${path.module}/user-data/cluster.tpl", {
